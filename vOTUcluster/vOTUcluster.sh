...
# 使用 getopts 处理命令行选项
while getopts ":i:r:o:d:t:rh" opt; do
  case $opt in
    i)
      INPUT_DIR=$OPTARG
      ;;
...
python filter_contigs.py 100 "${INPUT_DIR}" "${OUTPUT_DIR}/FilteredSeqs"
...
for FILE in $FILES; do
    ...
    echo -e "\n \n \n # 进行virus contigs交叉验证!!! \n \n \n"
    python CrossValid.py "$Genomad_dir" "$Viralverify_dir" "$Virsorter_dir" "$BASENAME" "$OUT_DIR"

    #Sequence提取
    python FilterRawResSeqs.py "$FILE" "$BASENAME" "$OUT_DIR"
    ...
    if [ ! -f "$OUT_DIR/Binning/vRhyme_results_${BASENAME}_filtered/log_vRhyme_${BASENAME}_filtered.log" ]; then
        #vRhyme bining
        nohup gnome-terminal -- bash -c "
            source ~/miniconda3/etc/profile.d/conda.sh;
    ...
    if [ "$REASSEMBLE" = true ]; then
        ...
        | python filter_reads_for_bin_reassembly.py "$OUT_DIR/Binning/vRhyme_results_${BASENAME}_filtered/vRhyme_best_bins_fasta" "$OUT_DIR/Binning/reads_for_reassembly" $STRICT_MAX $PERMISSIVE_MAX
        ...
        python concat_fasta_sequences.py $EXTRACTED_DIR "$OUT_DIR/Binning/Summary/tempsummary.fasta"
        ...
        python select_best_bins.py "$OUT_DIR/Binning/Summary/CheckRes/quality_summary.tsv" $EXTRACTED_DIR "$OUT_DIR/Binning/Summary/Finialfasta/Bestbins"
    ...
    python unbined.py -i "$OUT_DIR/Binning/vRhyme_results_${BASENAME}_filtered/vRhyme_best_bins_fasta" -r "$OUT_DIR/${BASENAME}_filtered.fasta" -o "$OUT_DIR/Binning/Summary/Finialfasta/unbined.fasta"
...
python concat_fasta_sequences.py "$OUTPUT_DIR/Summary/dRepRes/dereplicated_genomes" "$OUTPUT_DIR/Summary/Viralcontigs/Allbins.fasta"
...
python anicalc.py -i "${newDir}/merged_sequences_blast.tsv" -o "${newDir}/merged_sequences_ani.tsv"

#Finally, perform UCLUST-like clustering using the MIUVIG recommended-parameters (95% ANI + 85% AF):
python aniclust.py --fna "${newDir}/merged_sequences.fasta" --ani "${newDir}/merged_sequences_ani.tsv" --out "${newDir}/merged_sequences_clusters.tsv" --min_ani 95 --min_tcov 85 --min_qcov 0
...
python SelectCluster.py "${newDir}/merged_sequences.fasta" "${newDir}/merged_sequences_clusters.tsv" "$OUTPUT_DIR/Summary/Viralcontigs/drepviralcontigs.fa"

#Merge all
python Rename.py -i "$OUTPUT_DIR/Summary/Viralcontigs/drepviralcontigs.fa"
